...
FALLBACK_EMBEDDING_MODEL_PATH = str(BASE_DIR / "models" / "all-MiniLM-L6-v2")
...
try:
    ...
    print(f"  Failed to load primary embedder: {e}")
    print(f"   Falling back to {FALLBACK_EMBEDDING_MODEL_PATH}...")
    embedder = SentenceTransformer(
        FALLBACK_EMBEDDING_MODEL_PATH,
        device=DEVICE,
        local_files_only=True,
    )
...
# =========================
#   get_or_create_collection does NOT update the metric if the collection
...
def get_cosine_collection(
    ...
    if name in existing_names:
        ...
        if space != "cosine":
            print(
                f"   Collection '{name}' exists with metric='{space}'.\n"
...
def ingest_markdown() -> None:
    ...
    for md_path in tqdm(md_files, desc=" Markdown files"):
        ...
        if not chunks:
            print(f"  No chunks produced for {md_path.name} — skipping.")
            continue
...
def _post_ingest_validation() -> None:
    ...
    if sim < 0.90:
        print(
            "  WARNING: self-retrieval similarity is unexpectedly low (<0.90).\n"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--markdown-dir", default=None, help="Markdown directory to ingest")
