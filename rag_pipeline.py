...
# Ensure UTF-8 safe console output on Windows.
...
EMBED_PATH       = str(BASE_DIR / "models" / "BAAI-bge-large")
...
CROSS_ENCODER_PATH = str(BASE_DIR / "models" / "ms-marco-MiniLM-L-6-v2")
...
if device.type == "cuda":
    ...
    print("Device: cpu")
    print("NOTE: CUDA is not available in the current PyTorch build.")
    print("Install a CUDA-enabled PyTorch package and restart the app to use GPU.")
...
try:
    ...
    collection = None
    print(f"Warning: could not load default collection '{COLLECTION_NAME}': {e}")

# Helper for dynamic collections.
def get_collection(name: str):
    collection_names = [getattr(c, 'name', None) for c in client.list_collections()]
...
def collection_has_documents(name: str) -> bool:
    col = get_collection_if_exists(name)
    if not col:
        return False
    try:
        return col.count() > 0
...
def build_llm_prompt(question: str, qtype: str) -> str:
    format_instructions = {
        "definition": (
            "Write a clear, precise definition. "
            "Use exact terms from the question. Keep it concise (2-4 sentences)."
        ),
        "explanation": (
            "Answer in a clear paragraph. "
            "Cover the key details directly and concisely."
        ),
        "procedure": (
        ...
        "points": (
            "Answer using a numbered list. Each point must be a complete sentence."
        ),
        "list": (
            "List the relevant items directly and briefly."
        ),
        "comparison": (
            "Compare the items by highlighting their main differences and similarities."
        ),
        "general": (
            "Answer directly and helpfully with the information requested."
        ),
    }
    ...
    messages = [
        {
            "role": "system",
            "content": (
                "You are a helpful technical assistant. Answer the user's question directly and concisely.\n\n"
                "RULES:\n"
                "1. Be accurate and clear.\n"
                "2. Keep answers focused on the question.\n"
                "3. Use bullet or numbered format only when appropriate.\n"
...
def generate_llm_answer(question: str) -> str:
    ...
    prompt = build_llm_prompt(question, qtype)

    token_config = {
        "definition":  {"max_tokens": 40,  "temperature": 0.0, "top_p": 1.0},
        "explanation": {"max_tokens": 140, "temperature": 0.0, "top_p": 1.0},
        "procedure":   {"max_tokens": 150, "temperature": 0.0, "top_p": 1.0},
        "points":      {"max_tokens": 180, "temperature": 0.0, "top_p": 1.0},
        "list":        {"max_tokens": 220, "temperature": 0.0, "top_p": 1.0},
        "comparison":  {"max_tokens": 160, "temperature": 0.0, "top_p": 1.0},
        "general":     {"max_tokens": 220, "temperature": 0.0, "top_p": 1.0},
    }
    config = token_config.get(qtype, token_config["general"])
...
if use_finetuned:
    if device.type == "cuda":
        ...
        print(" Adapters found — loading on CPU (no quantization)...")
        base_model = AutoModelForCausalLM.from_pretrained(
            BASE_MODEL_PATH,
            dtype=torch.float32,
            local_files_only=True,
        ...
        model = PeftModel.from_pretrained(base_model, ADAPTER_PATH)
        print("Fine-tuned Qwen loaded on CPU")
else:
...
def biencoder_retrieve(
    ...
    try:
        results = collection.query(**{**query_args, "query_texts": [query]})
    except TypeError:
...
def retrieve_and_rerank(
    ...
    Args:
        ...
        skip_reranker : if True, skip cross-encoder and return bi-encoder results only
    """
    t0 = time.time()
    print(f"\nStage 1 — Bi-encoder retrieval (k={candidate_k})...")
    docs, bi_scores, _ = biencoder_retrieve(query, collection, k=candidate_k, verbose=verbose)
    t1 = time.time()
    print(f"  [Bi-encoder: {t1-t0:.2f}s]")

    if not docs:
        return [], []

    # If the bi-encoder candidate score is low, still proceed to rerank.
    # Broad questions like summarization often produce lower raw similarity,
    # but the cross-encoder can still find the best relevant chunks.
    if bi_scores and bi_scores[0] < 0.12:
        print(f"Very low bi-encoder score ({bi_scores[0]:.3f}) — query may be out of domain")
        return [], []

    if skip_reranker or not cross_encoder_available:
        print(f"Stage 2 — Skipping reranker (using bi-encoder scores only)...")
        return docs[:top_k], bi_scores[:top_k]
    ...
    t3 = time.time()
    print(f"  [Cross-encoder: {t3-t2:.2f}s]")
...
def retrieve_and_rerank_multiple(
    query: str,
    ...
    for collection in collections:
        c_docs, c_scores, _ = biencoder_retrieve(query, collection, k=candidate_k, verbose=verbose)
        docs.extend(c_docs)
    ...
    return crossencoder_rerank(query, docs, scores, top_k=top_k, verbose=verbose)
...
def build_prompt(
    ...
    format_instructions = {
        ...
        "points": (
            "Answer using a numbered list. Each point must start with a numeric label "
            "like 1., 2., 3. and be a complete, informative sentence drawn directly from the context.\n"
            "Format:\n1. [Point one]\n2. [Point two]\n3. [Point three]\n...\n"
            "Do NOT write paragraphs or bullets — use ONLY numbered points."
        ),
        "list": (
...
def generate_answer(
    ...
    token_config = {
        "definition": {"max_tokens": 48,  "temperature": 0.0, "top_p": 1.0},
        "explanation": {"max_tokens": 96,  "temperature": 0.0, "top_p": 1.0},
        "procedure":   {"max_tokens": 112, "temperature": 0.0, "top_p": 1.0},
        "points":      {"max_tokens": 96,  "temperature": 0.0, "top_p": 1.0},
        "list":        {"max_tokens": 112, "temperature": 0.0, "top_p": 1.0},
        "comparison":  {"max_tokens": 112, "temperature": 0.0, "top_p": 1.0},
        "general":     {"max_tokens": 120, "temperature": 0.0, "top_p": 1.0},
    }
    config = token_config.get(qtype, token_config["general"])
...
def ask(
    ...
    collection_name: Optional[Union[str, List[str]]] = COLLECTION_NAME,
) -> str:
    """
    Full RAG pipeline:
    ...
    if collection_name is None:
        ...
        if isinstance(collection_name, str):
            ...
            collection_names = [n for n in collection_name]
        ...
        for name in collection_names:
            try:
                ...
                print(f"Warning: skipping collection '{name}' due to error: {e}")
        ...
        if len(collections) == 1:
            ...
            docs, scores = retrieve_and_rerank_multiple(
                question,
                collections,
                candidate_k=candidate_k,
                top_k=top_k,
    ...
    # If the reranker judged the candidates to be too weak, avoid hallucinating.
    if scores:
        top_score = max(scores)
        if cross_encoder_available and top_score < -3.0:
            return "I don't have sufficient information in the knowledge base to answer this question."
        if not cross_encoder_available and top_score < 0.22:
            return "I don't have sufficient information in the knowledge base to answer this question."
...
if __name__ == "__main__":
    ...
    while True:
        ...
        answer = ask(q, candidate_k=8, top_k=2, threshold=0.30)
